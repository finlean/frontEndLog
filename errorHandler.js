...
// 拉取【忽略的域名】接口，因为目前存在跨域的问题，所以该代码被注释了。想要使用时，需要将上面两段代码注释掉，然后再取消本段代码的注释
// _ajax({
//     method: 'GET',
...
/**
 * api接口错误先行处理，当出现接口错误时，就执行该函数，收集接口的错误信息
 * @param {*接口返回的错误信息*} error
 */
 ...
 * 【code、api】异常收集处理函数
 * @param {错误信息对象} e
 * @param {当t存在时，表明是Vue.config.errorHandler捕获到的错误，因此要走【Vue 报错机制】；t不存在，走【Window 报错机制】} t
 */
const errorHandler = (e, t) => {
    console.error(e); // 将错误信息在控制台打印
    if (isIgnore) {
        if (isIgnore !== 'send') return;
    }
    let AgentInfo = _AgentInfo._init(); // 获取设备信息
    // 传给后台的错误信息对象，里面的字段，在数据库里面有描述
    let basicInfo = {
